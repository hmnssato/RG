...
lyr<-log10(yr);
sx<-(yr[length(yr[,1]),1]-yr[1,1])/(30-1);
xxx<-seq(yr[1,1],yr[length(yr[,1]),1],sx);
xxl<-log10(xxx);
llx = stats::smooth.spline(lyr[,1],lyr[,2],spar = 0);
yyl<-stats::predict(llx,xxl)$y;
xr_log <- cbind(xxl,yyl);
write.csv(xr_log,"dis_data_xr_sl_log.csv");
