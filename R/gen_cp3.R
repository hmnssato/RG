...
#generate semi-local cp (10, 10, 10)
...
for (k in 1:round(Nx*0.333,0)){
  for (l in 1:round(Nx*0.333,0)){
    if(k!=l){
      lx=abs(x[k]-x[l]);
...
for (k in (round(Nx*0.333,0)+1):round(Nx*0.333*2,0)){
  for (l in (round(Nx*0.333,0)+1):round(Nx*0.333*2,0)){
    if(k!=l){
      lx=abs(x[k]-x[l]);
      scc_sl = stats::predict(lls,lx)$y;
      lxx = scc_sl;
...
for (k in (round(Nx*0.333*2,0)+1):Nx){
  for (l in (round(Nx*0.333*2,0)+1):Nx){
    if(k!=l){
      lx=abs(x[k]-x[l]);
      scc_sl = stats::predict(lls,lx)$y;
      lxx = scc_sl;
...
xr3_log <- cbind(xxl,yyl);
write.csv(xr3_log,"dis_data_xr3_sl_log.csv");
